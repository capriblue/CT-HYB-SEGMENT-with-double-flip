...
void hybridization::sanity_check(const alps::params &parms)
{
  ...
  if (!parms.exists("cthyb.THERMALIZATION") ||
      ...
      !parms.exists("FLAVORS"))
    throw std::invalid_argument("please specify parameters THERMALIZATION, SWEEPS, and FLAVORS");

  // check paramater that are conditionally required
  if (parms["cthyb.MEASURE_freq"] && !parms.exists("NMATSUBARA"))
    throw std::invalid_argument("please specify parameter NMATSUBARA for # of Matsubara frequencies to be measured");

  if (parms["cthyb.MEASURE_legendre"] && !parms.exists("cthyb.N_LEGENDRE"))
    throw std::invalid_argument("please specify parameter N_LEGENDRE for # of Legendre coefficients to be measured");
  if (parms["cthyb.MEASURE_legendre"] && !parms.exists("NMATSUBARA"))
    throw std::invalid_argument("please specify parameter NMATSUBARA for # of Matsubara frequencies");
  if (parms["cthyb.MEASURE_nnt"] && !parms.exists("cthyb.N_nn"))
  ...
  if (parms["cthyb.MEASURE_nnw"] && !parms.exists("cthyb.N_W"))
  ...
  if (parms["cthyb.COMPUTE_VERTEX"])
  {
    ...
    if ((int)parms["NMATSUBARA"] < ((int)parms["cthyb.N_w2"] + (int)parms["cthyb.N_W"] - 1))
      throw std::invalid_argument("for computing the vertex, NMATSUBARA must be at least N_w2/2+N_W-1");
  }
...
void hybridization::show_info(const alps::params &parms, int crank)
{
  ...
  if (!crank)
  {
    ...
    if (parms["cthyb.MEASURE_freq"])
      std::cout << "measuring gw" << std::endl
                << "measuring fw" << std::endl;
    if (parms["cthyb.MEASURE_legendre"])
      std::cout << "measuring gl" << std::endl
                << "measuring fl" << std::endl;
    if (parms["cthyb.MEASURE_g2w"])
      std::cout << "measuring g2w" << std::endl;
    if (parms["cthyb.MEASURE_h2w"])
      std::cout << "measuring h2w" << std::endl;
    if (parms["cthyb.MEASURE_nn"])
      std::cout << "measuring nn" << std::endl;
    if (parms["cthyb.MEASURE_nnt"])
      std::cout << "measuring nnt" << std::endl;
    if (parms["cthyb.MEASURE_nnw"])
      std::cout << "measuring nnw" << std::endl;
    if (parms["cthyb.MEASURE_sector_statistics"])
      std::cout << "measuring sector statistics" << std::endl;
    if (parms["cthyb.COMPUTE_VERTEX"])
      std::cout << "vertex will be computed" << std::endl;
    if (parms.exists("cthyb.RET_INT_K"))
      std::cout << "using retarded interaction" << std::endl;
    if (parms.exists("U_MATRIX"))
      std::cout << "reading U matrix from file " << parms["U_MATRIX"] << std::endl;
    if (parms.exists("MU_VECTOR"))
      std::cout << "reading MU vector from file " << parms["MU_VECTOR"] << std::endl;
