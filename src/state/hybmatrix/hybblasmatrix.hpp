...
public:
  ...
  inline double &operator()(const int i, const int j){return *(values_+(i*memory_size_+j));}
  inline const double &operator()(const int i, const int j) const {return *(values_+(i*memory_size_+j));}
  //blas_matrix size
  ...
  inline void add_outer_product(int s, const double *v1, const double *v2, double alpha=1.){
    int inc=1;
    if(s>1){
      blas::dger_(&s, &s, &alpha,v2, &inc, v1, &inc, values_, &memory_size_); 
    }else if(s==1){
  ...
  /*inline void insert_row_column_last(blas::vector &row, blas::vector &col, double Mkk){
    ...
    int one=1;
    int oldsize=size_-1;
    blas::dcopy_(&oldsize, &(col(0)), &one, &(values_[oldsize     ]), &memory_size_); //copy in row (careful: col. major)
    blas::dcopy_(&oldsize, &(row(0)), &one, &(values_[oldsize*memory_size_]), &one         );   //copy in column
  ...
  inline void swap_column(int c1, int c2){
    if(c1==c2) return;
    blas::dswap_(&size_, &(values_[c1]), &memory_size_, &(values_[c2]), &memory_size_);
  }
  inline void swap_row(int c1, int c2){
    if(c1==c2) return;
    int one=1;
    blas::dswap_(&size_, &(values_[c1*memory_size_]), &one, &(values_[c2*memory_size_]), &one);
  }
  ...
  /*inline void right_multiply(const vector &v1, vector &v2) const{ //perform v2[i]=M[ij]v1[j]
    ...
    double alpha=1., beta=0.;    //no need to multiply a constant or add a vector
    int inc=1;
    blas::dgemv_(&trans, &size_, &size_, &alpha, values_, &memory_size_, v1.values_, &inc, &beta, v2.values_, &inc);
  }
  ...
  inline double determinant() const{
    ...
    //LU factorization
    lapack::dgesv_(&size_, &size_, &(det_blas_matrix[0]), &size_, &(ipiv[0]), &(identity[0]), &size_,&info);
    if(info < 0) {
  ...
  double max() const{
    ...
    if(size_<1) return 0;
    for(int i=0;i<size_;++i){
      rowmax_index=blas::idamax_(&size_, values_+i*memory_size_,&inc);
      rowmax[i]=*(values_+i*memory_size_+rowmax_index-1); //fortran convention: start counting from one
    }
    max_index=blas::idamax_(&size_, rowmax ,&inc);
    delete [] rowmax;
