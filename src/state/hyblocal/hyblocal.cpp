...
local_configuration::local_configuration(const alps::params &p) : U_(p),
                                                                  mu_(p),
                                                                  K_(p)
{
  beta_ = p["BETA"];
...
double local_configuration::local_weight_change(const segment &seg, int orb, bool antisegment) const
{
  ...
  if (length < 0)
    length += beta_; // wraparound segment
  ...
  if (use_retarded_interaction_)
  {
    ...
    if ((seg.t_start_ == 0) && (seg.t_end_ == beta_))
    { // not really a segment but a full line
      retarded_weight = 0.;
    }
    else
    {
      for (int i = 0; i < n_orbitals_; ++i)
      {
        for (std::set<segment>::const_iterator it = segments_[i].begin(); it != segments_[i].end(); ++it)
        {
          ...
          if ((it->t_start_ == seg.t_start_) || (it->t_start_ == seg.t_end_))
          { // segment remove
...
double local_configuration::find_next_segment_start_distance(double time, int orbital)
{
  if (segments_[orbital].size() == 0)
    return beta_; // no segments present
...
double local_configuration::find_next_segment_end_distance(double time, int orbital)
{
  ...
  if (it == segments_[orbital].end())
    ...
    distance = it->t_end_ - time;
  if (distance < 0)
    distance += beta_;
  // second possibility: the closest end time is part of the previous segment
  if (it == segments_[orbital].begin())
    it = segments_[orbital].end(); // wrap around
  it--;
  double distance2 = it->t_end_ - time;
  if (distance2 < 0)
    distance2 += beta_;
...
void local_configuration::insert_segment(const segment &new_segment, int orbital)
{
  ...
  {
    throw std::logic_error("insert segment start time could not be inserted.");
  }
  if (!times_set_.insert(new_segment.t_end_).second)
  {
    throw std::logic_error("insert segment end time could not be inserted.");
  }
}
void local_configuration::insert_antisegment(const segment &new_antisegment, int orbital)
{
  ...
  {
    throw std::logic_error("insert antisegment start time could not be inserted.");
  }
...
void local_configuration::remove_antisegment(const segment &new_antisegment, int orbital)
{
  ...
  else
  {
    ...
    if (it_earlier == segments_[orbital].begin())
      it_earlier = segments_[orbital].end(); // wrap around
    it_earlier--;
...
segment local_configuration::get_segment(int k, int orbital) const
{
  ...
  if (k >= (int)(segments_[orbital].size()))
    throw std::logic_error("not enough segments to get this one.");
  advance(it, k);
  return *it;
}
void local_configuration::remove_segment(const segment &new_segment, int orbital)
{
  ...
  if (segments_[orbital].erase(new_segment) == 0)
    throw std::logic_error("did not find segment to remove!");
  if (segments_[orbital].size() == 0)
    zero_order_orbital_occupied_[orbital] = false;
...
void local_configuration::check_consistency() const
{
  ...
  {
    if (order(i) < 2)
      continue; // nothing to check if none or only one segment present
    // std::cout<<"testing orbital: "<<i<<" order: "<<order(i)<<std::endl;
    for (std::set<segment>::const_iterator it = segments_[i].begin(); it != segments_[i].end(); ++it)
    {
      std::set<segment>::const_iterator next_it = it;
      next_it++;
...
{ // density on orbital i at time tauprime
  if (segments_[i].size() == 0)
    return zero_order_orbital_occupied_[i] ? 1. : 0.;
  else
  {
    // find first segment after tauprime, call it it_after
    ...
    if (it_after == segments_[i].end())
      it_after = segments_[i].begin();
    // find the segment which is before it_after. call it it_before
    std::set<segment>::const_iterator it_before = it_after;
    ...
    // find out the iterator it overlaps with a segment in this orbital. two cases: either it does not wrap and is just in between, or it wraps and is in between.
    bool occuppied = (it_before->t_end_ > it_before->t_start_ && (tauprime > it_before->t_start_ && tauprime < it_before->t_end_)) || (it_before->t_end_ < it_before->t_start_ && (tauprime < it_before->t_end_ || tauprime > it_before->t_start_));
...
{
  for (int i = 0; i < n_orbitals_; ++i)
    densities[i] += segment_density(i) * sign;
}
...
{
  if (i == j)
    return segment_density(i); // does not make much sense to call this function for i==j; also correct without this, but slow for i==j
  if (segments_[i].size() == 0)
  {
    if (zero_order_orbital_occupied_[i])
      return segment_density(j);
    else
      return 0.0;
  }
  else
  { // segments in orbital i
    if (segments_[j].size() == 0)
    {
      if (zero_order_orbital_occupied_[j])
        return segment_density(i);
      else
        return 0;
    }
    else
  ...
  for (int i = 0; i < n_orbitals_; ++i)
  {
    ...
    if (segments_[i].size() == 0)
    {
      if (zero_order_orbital_occupied_[i])
        ...
        std::fill(n_vectors[i].begin(), n_vectors[i].end(), 0);
    }
    else
    {
      std::fill(n_vectors[i].begin(), n_vectors[i].end(), 1);
      segment_container_t::const_iterator it = segments_[i].end();
      it--;
      if (it->t_end_ < it->t_start_)
        n_vectors[i][0] = 1; // last segment winds around the circle //n(0)=1
      else
        n_vectors[i][0] = 0; // n(0)=0
      ...
      {
        if (n_vectors[i][n] == -1)
          n_vectors[i][n] = 1 - n_vectors[i][n - 1]; // segment starts or ends -> occupation number changes
        else
          n_vectors[i][n] = n_vectors[i][n - 1]; // on the same segment -> occupation number identical to that of previous segment
      }
    }
  }
}
void local_configuration::measure_nnw(int i, std::vector<double> &nnw_re, double sign) const
{
  ...
  if (N_W == 1)
    return;
  for (segment_container_t::const_iterator it = segments_[i].begin(); it != segments_[i].end(); ++it)
  { // same contribution for winding segments
    double wm = 0;
...
void local_configuration::state_map_segment_insert(state_map &states, const segment &s, int index) const
{
  ...
  if (s.t_end_ <= s.t_start_)
    throw(std::string(__PRETTY_FUNCTION__) + " works only segments where the annihilator comes strictly after the creator");
  state_map::iterator next;
  int current_state = 0;

  next = states.upper_bound(s.t_start_);
  if (next == states.end())
  { // the new creator is the last operator
    if (!states.empty())
    { // there is at least one kink before (can be at exactly the same time) -> get it's state
      next--;
      current_state = next->second;
    }
    else
      current_state = 0; // no kink before, impurity is in state 0
  }
  else
  { // not past the end -> there is an operator with a later time
    if (next != states.begin())
    { // the new creator is not at the beginning, there is an earlier kink (can be at exactly the same time) -> get it's state
      next--;
      current_state = next->second;
    }
    else
    { // it's at the beginning, we'll be inserting the first one
      current_state = 0;
    }
  }
  // insert creator
  ...
  { // if there is an operator exactly at s.t_end_, it will be updated
    it->second += index;
  }
  ...
  if (next != states.begin())
  { // can never be states.begin(), we have inserted an operator at an earlier time!
    next--;
    current_state = next->second;
  }
  // insert annihilator
...
#include <iomanip>
void local_configuration::measure_sector_statistics(std::vector<double> &sector_statistics, double sign) const
{                   // complexity: linear in n_orbitals_
  ...
  for (int i = 0; i < n_orbitals_; ++i)
  {
    ...
    for (segment_container_t::const_iterator it = segments_[i].begin(); it != segments_[i].end(); ++it)
    {
      ...
      { // winding segment
        ...
        state_map_segment_insert(states, *it, index);
    }
  }
